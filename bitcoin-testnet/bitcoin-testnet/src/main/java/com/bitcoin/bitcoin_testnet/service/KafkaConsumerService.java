...
public class KafkaConsumerService {
    ...
    // 4. Annulation automatique des transactions AWAITING_CONFIRMATION expirées (toutes les 5 min)
    @Scheduled(fixedDelay = 300000)
    public void cancelExpiredAwaitingConfirmations() {
        LocalDateTime now = LocalDateTime.now();
        List<Transaction> expired = transactionRepository.findByStatusAndConfirmationExpiresAtBefore(
                "AWAITING_CONFIRMATION", now);
        for (Transaction tx : expired) {
            tx.setStatus("FAILED");
            transactionRepository.save(tx);
            log.info("⏰ AWAITING_CONFIRMATION expirée annulée automatiquement : {}", tx.getId());
        }
    }
        ...
        try {
            ...
            if (responseCode == 200) {
                ...
                if (confirmations != tx.getConfirmations()) {
                    ...
                    if (!previousStatus.equals(tx.getStatus())) {
                        saveNotification(tx);
                    }
                }
            } else if (responseCode == 404 && "PENDING".equals(tx.getStatus())) {
                tx.setStatus("FAILED");
                transactionRepository.save(tx);
                log.warn("❌ Transaction {} introuvable sur la blockchain — FAILED", txId);
                saveNotification(tx);
            }
        } catch (Exception e) {
    ...
    private void saveNotification(Transaction tx) {
        walletRepository.findByAddress(tx.getFromAddress()).ifPresent(wallet -> {
            Notification notif = new Notification();
            notif.setUserId(wallet.getUserId());
            notif.setType("TX_UPDATE");
            notif.setTxId(tx.getTxId());
            notif.setRead(false);
            notif.setCreatedAt(new Date());
            if ("CONFIRMED".equals(tx.getStatus())) {
                notif.setMessage("✅ Transaction confirmée (" + tx.getConfirmations() + " conf.) : " + tx.getTxId());
            } else if ("FAILED".equals(tx.getStatus())) {
                notif.setMessage("❌ Transaction échouée : " + tx.getTxId());
            }
            notificationRepository.save(notif);
            log.info("🔔 Notification créée pour utilisateur {}", wallet.getUserId());
        });
    }
