...
export class MfaSetupComponent implements OnInit {
  ...
  confirmSetup() {
    if (!this.confirmCode || this.confirmCode.length !== 6) {
      this.message = 'Entrez le code à 6 chiffres de Google Authenticator';
      return;
    }
    ...
    this.bitcoinService.verifyMfa(this.confirmCode, preAuthToken).subscribe({
      ...
      error: (err) => {
        this.isVerifying = false;
        this.message = err.error?.message || 'Code invalide. Vérifiez Google Authenticator et réessayez.';
        this.confirmCode = '';
      }
    });
  }
